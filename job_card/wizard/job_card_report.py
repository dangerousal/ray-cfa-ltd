...
class JobCardRport(models.TransientModel):
    ...
    def print_job_card_report(self):
        datas = {'id': self, 'form': self.read()[0], 'line_ids': [], 'report_type': self.report_type}
        if self.report_type == 'dailay_report' and self.date:
            line_ids = self.env['account.analytic.line'].search([('date', '=', self.date)])
            datas.update({
                'date': self.date,
        ...
        if self.report_type == 'project_report' and self.project_id:
            ...
            line_ids = self.env['account.analytic.line'].search([('account_id', 'in', account_ids)])
            datas.update({
